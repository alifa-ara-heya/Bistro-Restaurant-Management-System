...
app.use(
    cors({
        origin: [
            "http://localhost:5173",
            "https://bistro-restaurant-alifa.web.app",
            "bistro-restaurant-alifa.firebaseapp.com",
        ],
        credentials: true,
    })
);
...
async function run() {
    try {
        ...
        app.get('/admin-stats', verifyToken, verifyAdmin, async (req, res) => {
            ...
            //this is not the best way to get revenue, because it loads all the data
            ...
            const result = await paymentCollection.aggregate([
                {
                    $group: {
                        _id: null, //everything will be grouped
        ...
        app.get('/order-stats', verifyToken, verifyAdmin, async (req, res) => {
            const result = await paymentCollection.aggregate([
                ...
                },
                {
                    $project: {
                        _id: 0,
                        category: '$_id', //to rename the _id as category
                        quantity: '$quantity',
                        revenue: '$revenue'
                    }
                }
